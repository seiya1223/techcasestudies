...
## Customer Profile ##
Hark is a technology company, based in the United Kingdom, building an interconnected cloud-based sensor platform that allows users to monitor, store and gain insight into their environmental data in real-time. Alongside their own hardware, they allow any industry standard sensor to connect to our platform with an almost plug and play nature for rapid deployments.
...
<img src="../images/hark/PublishAPI.PNG" width="700">
...
#### Testing
As part of our release pipeline we used Postman ([https://www.getpostman.com](https://www.getpostman.com)) to create our tests:

<img src="../images/hark/postman_write_test.png" width="700"/>

These took the form of calls to the API and testing the responses that were returned. These tests were saved as a collection and could be run within the UI and also exported as a json file. 
...
To use the Postman tests in the release pipeline we used Newman [https://github.com/postmanlabs/newman](https://github.com/postmanlabs/newman), which is a command line test runner for Postman. We added an ```npm install newman``` task to the pipeline and from a command line task called newman to execute the tests. We  also redirected the output to be a jUnit test result:
...
# Learnings
Other than the many technical learnings documented here we came away with a couple of more genral insights:
- There are often multiple ways to reach a desired goal, especially when designing release pipelines.
- Value Stream Mapping is not always appropriate. This was the case for Hark as they are a startup who had few defined processes. This has now changed as a result of the hackfest. It would be interesting to carry out a VSM in 6 or 12 months time once processes are in place.
